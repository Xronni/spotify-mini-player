...
Download `spotify-mini-player_1.3.0_all.deb` from the [Releases page](https://github.com/Xronni/spotify-mini-player/releases/latest) and install:
```bash
sudo dpkg -i spotify-mini-player_1.3.0_all.deb
sudo apt install -f  # automatically install dependencies if needed
```

#### Option B: Standalone Archive or Git Clone
1. Clone the repository or download `spotify-mini-player-v1.3.0-linux-x86_64.tar.gz`:
   ```bash
...
Скачайте `spotify-mini-player_1.3.0_all.deb` со страницы [Релизов](https://github.com/Xronni/spotify-mini-player/releases/latest) и выполните:
```bash
sudo dpkg -i spotify-mini-player_1.3.0_all.deb
...
1. Клонируйте репозиторий или скачайте архив `spotify-mini-player-v1.3.0-linux-x86_64.tar.gz`:
   ```bash
   git clone https://github.com/Xronni/spotify-mini-player.git
   cd spotify-mini-player
   ```
